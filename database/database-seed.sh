...
mysql -h"$MYSQL_HOST" -u"$MYSQL_ROOT" -p"$MYSQL_ROOT_PASSWORD" -P"$MYSQL_PORT" "$DB_NAME" < "$INITIAL_SQL_SCRIPT"
if [ $? -eq 0 ]; then
...
mysql -h"$MYSQL_HOST" -u"$MYSQL_ROOT" -p"$MYSQL_ROOT_PASSWORD" -P"$MYSQL_PORT" "$DB_NAME" -e "
CREATE USER '$MYSQL_USER'@'%' IDENTIFIED BY '$USER_PASSWORD';
GRANT INSERT, UPDATE, DELETE, SELECT ON $DB_NAME.* TO '$MYSQL_USER'@'%';"
if [ $? -eq 0 ]; then
...
for sql_file in "$SEED_SQL_SCRIPTS_PATH"/*.sql; do
    ...
    if [ -e "$sql_file" ]; then
        ...
        mysql -h"$MYSQL_HOST" -u"$MYSQL_ROOT" -p"$MYSQL_ROOT_PASSWORD" -P"$MYSQL_PORT" "$DB_NAME" < "$sql_file"
        if [ $? -eq 0 ]; then
